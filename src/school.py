...
def detect_schools(ds_Sv: xr.Dataset, config: dict) -> xr.DataArray:
    ...
    xr.DataArray
        布尔 mask，True 表示鱼群区域
    """
    # 直接加载子模块文件，绕过 echopype 顶层 __init__ 的 Windows YAML 编码问题
    import importlib.util
    from pathlib import Path

    _ep_spec = importlib.util.find_spec("echopype")
    if _ep_spec is None or _ep_spec.origin is None:
        raise ImportError("echopype 未安装")
    _ep_root = Path(_ep_spec.origin).parent
    ...
    shoal_echoview = _mod.shoal_echoview
    ...
    if "depth" in ds_Sv:
        idim_center = ds_Sv["depth"].isel(ping_time=0).values
    elif "echo_range" in ds_Sv:
        idim_center = ds_Sv["echo_range"].isel(ping_time=0).values
    else:
    ...
    mask = shoal_echoview(ds_Sv, **params)
