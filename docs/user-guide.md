...
3. 点击"网格分析"
4. 结果自动叠加到回波图：
   - **颜色编码**：每个网格单元根据 mean_sv 值使用 colormap 着色（半透明，alpha=0.35）
   - **白色边框**：网格单元边界用白色细线标出
   - **数值标注**：单元格足够大时显示 mean_sv 数值
   - 叠加层位于 echogram 之上、底线/鱼群之下，不影响其他叠加显示
5. 统计对话框显示详细数据，支持排序、过滤和导出
...
2. 自动检查：
   ...
   - 底线有效性（跳变、连续性）
3. 弹出质量检查对话框，包含：
   - **状态指示**：顶部状态栏显示整体结果 — 绿色(通过) / 黄色(警告) / 红色(失败)
   - **Sv 数据质量**：值范围 (dB)、数据尺寸 (pings × samples)、NaN 比例
   - **底线质量**：有效 ping 数、NaN 比例（未检测底线时隐藏）
   - **警告列表**：列出所有发现的问题，无警告时显示绿色提示

### 4.6 多频分析

1. 属性面板 → 处理参数 → "📊 多频分析"
2. 自动分析并弹出多频分析对话框，包含两个标签页：
   - **通道摘要**：显示所有通道的名称、频率 (Hz)、Ping 数、采样点数
   - **频率对比**：多频率 ABC 对比分析（需 ≥ 2 通道），显示 mean_abc、std_abc、max_abc
   - 少于 2 个通道时，频率对比标签页显示灰色提示
3. 支持导出：
   - CSV 导出当前标签页数据
   - Excel 导出全部标签页
   - 复制到剪贴板
...
| 网格叠加不显示 | 确认已运行网格分析，检查 mean_sv 列存在；网格使用半透明着色，确保 echogram 已加载数据 |
